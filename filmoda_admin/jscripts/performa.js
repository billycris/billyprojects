	...
	$.performa= {
		...
		display_performa:function(){
			$("#tbl_perfoma").flexigrid({
				...
				dataType : 'json',
				colModel :[ 
					{display : 'Buyer',name : 'product_photo',width : 170,sortable : false,align : 'center'}, 
					{display : 'Invoice',name : 'product_code',width : 270,sortable : true,align : 'center'}, 
					// {display : 'Total Amount',name : 'product_name',width : 140,sortable : true,align : 'left'},
					// {display : 'Discount Price',name : 'item_description',width : 160,sortable : false,	align : 'left',hide : false}, 
					{display : 'Shipment Date',name : 'product_name',	width :178,sortable : false,align : 'center'},
					{display : 'Order Date',name : 'product_name',	width : 160,sortable : false,align : 'center'},
					{display : 'Status',name : 'product_name',	width : 115,sortable : false,align : 'center'}
				],
				buttons : [ 
			...
			function action_performa(com,grid){
				switch(com){
					case 'Add':
							$("#product_action_header").html("<b>Add New Proforma Invoice</b>");
							$.performa.clear_fields();
							$("div.add_performa").show();
							$("div.performa").hide();
							$("#c_add_save").show();
							$("div.add_performa").find("#p_mPurDate").mask("9999-99-99");
							$("div.add_performa").find("#p_mRecDate").mask("9999-99-99");
							$("div.proforma_add_item :input").attr('disabled', true);
							$("div.proforma_entry :input").attr('disabled', false);
							$("div.invoice").hide();
							$.performa.cancel_save();
							$.performa.save_invoice();
							// $.performa.displayInvoice();
					...
					case 'Edit':
						if($('.trSelected',grid).length>0){
							$.performa.clear_fields();
							$("#product_action_header").html("<b>Edit Proforma Invoice Item</b>");
							$("div.proforma_add_item :input").attr('disabled', false);
							$("div.proforma_entry :input").attr('disabled', true);
							$("div.add_performa").show();
							$("div.invoice").show();
							$("div.performa").hide();
							// $("#c_add_save").show();
							var prof = $('.trSelected',grid);
							var prof_id = prof[0].id.substr(3);
							var profid = prof[0].id;
							
							var pDate = $("#"+profid).find("td").eq(2).text();
							$("#p_mPurDate").val(pDate);	
							$("#p_mRecDate").val($("#"+profid).find("td").eq(3).text());
							
							$.ajax({
								type:'POST',
								url:'action/fill_performa_invoice_item.php',
								data:{prof_id:prof_id},
								dataType:'json',
								success:function(d){
									if(d.e){
										$("#buyer_id").val(d.bcode);
										$.performa.invoiceTable(prof_id);
									}else{
							...
							$.performa.add_sub_item(prof_id,pDate);
						}else{
					...
					case 'Delete':
							if($('.trSelected',grid).length>0){
								var prof = $('.trSelected',grid);
								var prof_id = prof[0].id.substr(3);
								var ans = confirm('Warning!! Deleting this data may loss all your item under this data.');
								if(ans){
									$.ajax({
										type:'POST',
										url:'action/delete_proforma_invoice_item.php',
										data:{prof_id:prof_id},
										dataType:'json',
										success:function(d){
											if(d.e){
												alert(d.m);
												$("#tbl_perfoma").flexReload();
											}else{
												alert(d.m);
											}
										},
		...
		save_invoice:function(){
			$("div.mws-button-row").find("#c_add_save").unbind('click').click(function(){
				var b_id = $("#buyer_id").val();
				// var b_id = $("#p_mQuant").val();
				var pur_date = $("#p_mPurDate").val();
				var rec_date = $("#p_mRecDate").val();
				if(b_id.length < 1 || pur_date.length < 1 || rec_date.length<1){
					alert('Please fill up all required fields.');
				}else{
					$.ajax({
						type:'POST',
						url:'action/add_performa_invoice.php',
						data:{
							b_id  	  : b_id,
							pur_date  : pur_date,
							rec_date  : rec_date
						...
						success:function(d){
							if (d.e) {
								// alert(d.cDate);
								$("div.proforma_add_item :input").attr('disabled', false);
								// $("div.add_performa").hide();
								// $("div.performa").show();
								$("div.invoice").show();
								$("#tbl_perfoma").flexReload();
								$.performa.add_sub_item(d.pro_id,d.cDate);
								$.performa.invoiceTable(d.pro_id);
							}else{
								alert('failed');
							}
						},
		...
		add_sub_item: function(id,cDate){
			$("div.mws-button-row").find("#add_item").unbind('click').click(function(){
				var item_id = $("#item_id").val();
				var item_quan =$("#p_mQuant").val();
				
				if(item_id.length < 1 || item_quan.length < 1 ){
					alert('Please fill up all required fields.');
				}else{
					$.ajax({
						type:'POST',
						url:'action/add_sub_proforma_invoice.php',
						data:{
							id 			: id,
							cDate 		: cDate,
							item_id 	: item_id,
		...
		displayInvoice:function(){
			$("div.mws-button-row").find("#c_view_item").unbind('click').click(function(){
				$("div.invoice").show();
				// $.performa.invoiceTable();
			});
		},
		invoiceTable:function(proforma_id){
			$(".invoice").html('');
			$(".invoice").append('<table id = "tbl_invoice"></table>');
			
			$("#tbl_invoice").flexigrid({
				url :'action/display_proforma_sub_invoice.php' + '?prof_id=' + proforma_id ,
				dataType : 'json',
				colModel :[ 
					{display : 'Item Code',name : 'product_photo',width : 168,sortable : false,align : 'center'}, 
					{display : 'Item Name',name : 'product_photo',width : 168,sortable : false,align : 'center'}, 
					{display : 'Price',name : 'product_code',width : 90,sortable : true,align : 'center'}, 
					{display : 'Quatity',name : 'product_code',width : 95,sortable : true,align : 'center'}, 
					{display : 'Total Amount',name : 'product_name',width : 100,sortable : true,align : 'left'},
				],
				buttons : [ 
				...
				// searchitems : [ 
					...
					// {display : 'Status',name : 'product_name',isdefault : true}
				// ],
				sortname : "proforma_id",
				sortorder : "desc",
				usepager : true,
				resizable: false,
				title : 'Performa Invoice Item',
				useRp : true,
				rp : 15,
				showTableToggleBtn : true,
				width : 500,
			...
			function action_invoice(com,grid){
				switch(com){
					case 'Add':
						// alert('add');
						$("div.add_invoice_item").show();
						break;
					case 'Delete':
							if($('.trSelected',grid).length>0){
								$("#c_cancel_save").html('Back to table.');
								var profItem = $('.trSelected',grid);
								var profItem_id = profItem[0].id.substr(3);
								var ans = confirm('Are you sure you want to delete this item?');
								if(ans){
									$.ajax({
										type:'POST',
										url:'action/delete_sub_proforma_invoice_item.php',
										data:{profItem_id:profItem_id},
										dataType:'json',
										success:function(d){
											if(d.e){
												alert(d.m);
...
jQuery(document).ready(function(){ jQuery.performa.init(); });